...
class TritonPythonModel:
    ...
    def execute(self, requests):
        ...
        output2_dtype = self.output1_dtype
        ...
        for request in requests:
            ...
            # try: # ValueError
            model_outputs = [in_0.as_numpy().astype(input0_dtype).reshape(-1),
                             in_1.as_numpy().astype(input1_dtype).reshape(-1),
                             in_2.as_numpy().astype(input2_dtype).reshape(-1)]
            # print('model outputs:', model_outputs)
            print(f"{orig_size} | {post_size}")
            boxes, scores, labels = Processing.postprocess_yolo(model_outputs, post_size, orig_size,
                                                                conf_th=0.5,  nms_threshold=0.5)
            # raise ValueError(f"{boxes} | {scores} | {labels} | {orig_size} | {post_size}")
